...
private[scheduler] class DecommissionTracker (
  ...
  def this(sc: SparkContext,
           ...
           dagScheduler: Option[DAGScheduler]) = {
    this(sc.listenerBus, sc.conf, client, dagScheduler)
  }
  ...
  def addNodeToDecommission(hostname: String, terminationTimeMs: Long,
    ...
    // if delay is less than a minute than decommission immediately
    if (terminationTimeMs - curTimeMs < minDecommissionTime * 1000) {
      ...
      reason match {
        case SpotRotationLoss | SpotLoss =>
...
private[spark] object DecommissionTracker extends Logging {
  ...
  def getNodeDecommissionReason(reason: String): NodeDecommissionReason = {
    reason match {
      case "nodeLoss" => SpotLoss
      case "nodeRotationLoss" => SpotRotationLoss
...
@DeveloperApi
private[spark] case object SpotLoss extends NodeDecommissionReason {
...
@DeveloperApi
private[spark] case object Rebalance extends NodeDecommissionReason {
  override def message: String = "rebalance"
}
